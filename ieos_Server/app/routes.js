    ...
    app.post('/mobevoice', isLoggedIn, function (req, res) {
        httpreq.post(parser, { body: req.body.speechresult }, function (err, http_res, body) {
            ...
            if (feedback.success == true) {
                if (req.user.local.allow_devices.indexOf(feedback.device_name) != -1) {
                    //send http request
                    httpreq.post("http://140.112.42.154:5566", { body: JSON.stringify(feedback) }, function (err, http_res, body) {
                        res.send({ "msg": "Successful" });
                    });
                }
                else {
                    res.send({ "msg": "Permission deny" });
                }
            }
            else {
